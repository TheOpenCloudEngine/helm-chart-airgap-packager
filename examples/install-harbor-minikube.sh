#!/usr/bin/env bash
# Install Harbor 2.14.2 from chart saved by load-harbor-minikube.sh
#
# Usage:
...
#   - helm CLI installed
#   - Images loaded by load-harbor-minikube.sh
#
...
# ── Locate chart ──────────────────────────────────────────────────────────────
CHART_TGZ=$(find "$CHART_DIR" -name "harbor-*.tgz" 2>/dev/null | sort -V | tail -1)
if [ -z "$CHART_TGZ" ]; then
  echo "ERROR: No harbor chart .tgz found in $CHART_DIR"
  echo "       Run load-harbor-minikube.sh first."
  exit 1
fi
